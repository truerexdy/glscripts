...
delete_share() {
    while true; do
        ...
        share_dir_prompt="/srv/samba/$share"

        read -r -p "Delete share directory? (y/n): " del_dir
        if [ "$del_dir" == "y" ]; then
            read -r -p "Enter share directory path to delete or press enter for $share_dir_prompt: " dir_path
            dir_path=${dir_path:-$share_dir_prompt}
            rm -rf "$dir_path"
...
create_share() {
    while true; do
        ...
        local share_dir="/srv/samba/$share"
